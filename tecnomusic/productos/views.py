from django.shortcuts import render, redirect, get_object_or_404 # Inportamos las funciones necesarias de Django
from django.core.paginator import Paginator, EmptyPage, PageNotAnInteger # Importamos las clases de paginación
...
def lista_productos(request): # Vista para listar productos
   ...
   paginator = Paginator(productos_list, 10) # Paginamos los productos, mostrando 10 por página
   ...
   return render(request, 'productos/product_list.html', {'productos': productos}) # Renderizamos la plantilla con los productos paginados

def create_product(request): # Vista para crear un nuevo producto
   form = ProductoForm(request.POST or None) # Creamos una instancia del formulario con los datos POST si existen o None si no hay datos
...
def edit_product(request, pk): # Vista para editar un producto existente
   prod = get_object_or_404(Producto, pk=pk) # Obtenemos el producto por su clave primaria, o devolvemos un error 404 si no existe
   form = ProductoForm(request.POST or None, instance=prod) # Creamos una instancia del formulario con los datos POST si existen, o None si no hay datos, y si no, y vinculamos el producto existente
   if request.method == 'POST' and form.is_valid():# Si el método es POST y el formulario es válido
...
def inicio(request):
   """Vista para la página de inicio de Tecnomusic."""
   context = {
       'tienda': 'Tecnomusic',
...
def inicio(request):
   context = {'tienda': 'Tecnomusic', 'descripcion': 'Venta de productos musicales'}
